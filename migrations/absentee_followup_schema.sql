...
-- Update notification templates to include absentee follow-up (idempotent)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='notification_templates') THEN
    IF NOT EXISTS (SELECT 1 FROM notification_templates WHERE church_id=1 AND name='Absentee Follow-up Assignment') THEN
      INSERT INTO notification_templates (church_id, name, channel, subject, body, description, variables, is_default, created_by)
      VALUES (1, 'Absentee Follow-up Assignment', 'email', 'Follow-up Assignment: {{member_name}}', '<h2>Follow-up Assignment</h2><p>Dear {{assignee_name}},</p><p>You have been assigned to follow up with <strong>{{member_name}}</strong> who has been absent for {{consecutive_absences}} consecutive meetings.</p><p><strong>Last attendance:</strong> {{last_attendance_date}}</p><p><strong>Reason:</strong> {{reported_reason}}</p><p><strong>Due date:</strong> {{due_date}}</p><p>Please contact them using the preferred method and update the follow-up record.</p><p>Cell Leader: {{cell_leader_name}}</p>', 'Email notification for follow-up assignments', '["assignee_name", "member_name", "consecutive_absences", "last_attendance_date", "reported_reason", "due_date", "cell_leader_name"]', true, NULL);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM notification_templates WHERE church_id=1 AND name='Absentee Follow-up Reminder') THEN
      INSERT INTO notification_templates (church_id, name, channel, subject, body, description, variables, is_default, created_by)
      VALUES (1, 'Absentee Follow-up Reminder', 'email', 'REMINDER: Pending Follow-up for {{member_name}}', '<h2>Follow-up Reminder</h2><p>Dear {{assignee_name}},</p><p>This is a reminder that you have a pending follow-up for <strong>{{member_name}}</strong>.</p><p><strong>Status:</strong> {{status}}</p><p><strong>Due date:</strong> {{due_date}}</p><p><strong>Last contact:</strong> {{last_contact_date}}</p><p>Please complete this follow-up as soon as possible.</p><p>Cell Leader: {{cell_leader_name}}</p>', 'Reminder for pending follow-ups', '["assignee_name", "member_name", "status", "due_date", "last_contact_date", "cell_leader_name"]', true, NULL);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM notification_templates WHERE church_id=1 AND name='Overdue Absentee Alert') THEN
      INSERT INTO notification_templates (church_id, name, channel, subject, body, description, variables, is_default, created_by)
      VALUES (1, 'Overdue Absentee Alert', 'email', 'OVERDUE: Urgent Follow-up Required for {{member_name}}', '<h2>Urgent: Overdue Follow-up</h2><p>Dear {{cell_leader_name}},</p><p>The follow-up for <strong>{{member_name}}</strong> is now OVERDUE.</p><p><strong>Originally due:</strong> {{due_date}}</p><p><strong>Days overdue:</strong> {{days_overdue}}</p><p><strong>Assigned to:</strong> {{assignee_name}}</p><p>Please ensure this follow-up is completed urgently.</p><p>Cell Leader: {{cell_leader_name}}</p>', 'Alert for overdue follow-ups', '["cell_leader_name", "member_name", "due_date", "days_overdue", "assignee_name"]', true, NULL);
    END IF;
  END IF;
END $$; 