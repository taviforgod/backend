  ...
  try {
    await db.query(
      ...
      [
        church_id,
        user.id,
        null, // member_id will be set when member record is created
        JSON.stringify({ in_app: true, email: !!email, sms: !!phone, whatsapp: false }),
        true, // digest_enabled
        JSON.stringify({ start: "22:00", end: "08:00" }) // Default quiet hours
      ]
    );
  } catch (error) {
    console.warn('Failed to create default notification preferences for user:', user.id, error);
