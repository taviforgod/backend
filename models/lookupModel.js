...
// Zones CRUD
export async function getZones(church_id) {
  const { rows } = await db.query(
    'SELECT id, name, description, active FROM zones WHERE church_id = $1 ORDER BY name',
    [church_id]
  );
  return rows;
}

export async function addZone({ church_id, name, description, active = true }) {
  const { rows } = await db.query(
    'INSERT INTO zones (church_id, name, description, active) VALUES ($1, $2, $3, $4) RETURNING *',
    [church_id, name, description, active]
  );
  return rows[0];
}

export async function updateZone(id, { name, description, active }) {
  const { rows } = await db.query(
    'UPDATE zones SET name = $1, description = $2, active = $3 WHERE id = $4 RETURNING *',
