...
async function generateGivingSummaryReport(churchId, params) {
  ...
  return {
    ...
    summary: {
      ...
      active_givers: Math.max(...rows.map(row => parseInt(row.active_givers))),
      average_gift: rows.length > 0 ? rows.reduce((sum, row) => sum + parseFloat(row.average_gift), 0) / rows.length : 0
    },
    monthly_data: rows,
  ...
  return {
    ...
    attendance: {
      summary: {
        ...
        active_cells: Math.max(...attendanceResult.rows.map(row => parseInt(row.active_cells || 0)))
      },
...
async function generateDiscipleshipReport(churchId, params) {
  ...
  const foundationQuery = `
    SELECT
      ...
      AVG(fs.module_number) as avg_progress,
      COUNT(CASE WHEN fs.next_session_date < CURRENT_DATE THEN 1 END) as overdue_sessions
  ...
  const baptismPrepQuery = `
    SELECT
      ...
      AVG(bp.completion_percentage) as avg_completion,
      COUNT(CASE WHEN bp.baptism_date IS NOT NULL THEN 1 END) as baptized_count
    FROM baptism_prep_checklist bp
    WHERE bp.church_id = $1
    GROUP BY bp.status
  `;
...
async function generatePersonalGrowthReport(churchId, params) {
  ...
  const growthPlansQuery = `
    SELECT
      ...
      COUNT(CASE WHEN pgp.end_date < CURRENT_DATE THEN 1 END) as overdue_plans
...
async function generateOutreachReport(churchId, params) {
  ...
  const eventsQuery = `
    SELECT
      ...
      AVG(oe.impact_rating) as avg_impact_rating
  ...
  const baptismQuery = `
    SELECT
      ...
      COUNT(CASE WHEN bc.status = 'baptized' THEN 1 END) as baptized_candidates,
      COUNT(CASE WHEN bc.baptism_method = 'immersion' THEN 1 END) as immersion_baptisms,
      COUNT(CASE WHEN bc.baptism_method = 'sprinkling' THEN 1 END) as sprinkling_baptisms
