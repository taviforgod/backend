...
export async function getAllCrisisFollowups(filters = {}) {
  ...
  if (church_id) { clauses.push(`church_id = $${i++}`); values.push(church_id); }
  if (typeof is_active === 'boolean') { clauses.push(`is_active = $${i++}`); values.push(is_active); }
  if (crisis_type) { clauses.push(`crisis_type ILIKE $${i++}`); values.push(`%${crisis_type}%`); }
