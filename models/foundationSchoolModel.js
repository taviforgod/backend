...
export async function getFoundationEnrollments(churchId, filters = {}) {
  ...
  if (filters.member_id) {
    query += ` AND fe.member_id = $${paramIndex}`;
...
export async function updateEnrollmentProgress(enrollmentId, data) {
  ...
  const current = await db.query(
    'SELECT status FROM foundation_school_enrollments WHERE id = $1',
    [enrollmentId]
  );
  if (current.rows.length === 0) return null;
  const currentStatus = current.rows[0].status;

  if (currentStatus === 'completed' && (status !== undefined || level !== undefined || notes !== undefined)) {
    const err = new Error('Enrollment is completed and locked');
    err.code = 'FOUNDATION_COMPLETED_LOCK';
    throw err;
  }
