...
export async function listNotifications({ church_id, user_id, member_id, page = 0, limit = 20, status, channel, q }) {
  const wheres = ['church_id = $1'];
  const params = [church_id];
  let idx = 2;
  if (user_id || member_id) {
    wheres.push('(user_id = $' + idx + ' OR member_id = $' + idx + ')');
    params.push(user_id || member_id);
    idx++;
  }
  if (status === 'read') wheres.push('read = TRUE');
  if (status === 'unread') wheres.push('read = FALSE');
  if (channel) { wheres.push('channel = $' + idx); params.push(channel); idx++; }
  if (q) { wheres.push('(title ILIKE $' + idx + ' OR message ILIKE $' + idx + ')'); params.push('%' + q + '%'); idx++; }
  const where = 'WHERE ' + wheres.join(' AND ');
  const totalRes = await db.query(`SELECT count(*) AS total FROM in_app_notifications ${where}`, params);
  const total = Number(totalRes.rows[0]?.total || 0);
  params.push(limit, page * limit);
  const rowsRes = await db.query(
    `SELECT * FROM in_app_notifications ${where} ORDER BY created_at DESC LIMIT $${idx++} OFFSET $${idx++}`,
    params
  );
