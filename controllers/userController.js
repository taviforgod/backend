    ...
    (async () => {
      try {
        ...
        const userRes = await db.query(
          'SELECT name, email FROM users WHERE id = $1 AND church_id = $2',
          [deletedUserId, church_id]
        );
        const deletedUser = userRes.rows[0];
        const userDisplayName = deletedUser?.name || deletedUser?.email || `User ${deletedUserId}`;
        
        const title = 'User deleted';
        const message = `User "${userDisplayName}" was deleted.`;
  ...
  try {
    ...
    (async () => {
      try {
        ...
        const message = `User "${userDisplayName}" was activated.`;
  ...
  try {
    ...
    (async () => {
      try {
        ...
        const message = `User "${userDisplayName}" was deactivated.`;
  ...
  try {
    ...
    (async () => {
      try {
        ...
        // Get user details for notification (user object already fetched above)
        const userDisplayName = user?.name || user?.email || `User ${id}`;
        
        const title = 'User locked';
        const message = `User "${userDisplayName}" account was locked.`;
