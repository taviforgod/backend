  ...
  try {
    ...
    for (const row of rows) {
      ...
      const membersRes = await db.query(
        `SELECT cm.*, mb.first_name, mb.surname, mb.contact_primary, mb.email
...
export async function getLastAttendance(req, res) {
  try {
    ...
    const result = await db.query(
      `SELECT 
          COALESCE(wr.total_cell_attendance, jsonb_array_length(COALESCE(wr.attendees, '[]'::jsonb))) AS present_count,
          COALESCE(wr.absentees_count, jsonb_array_length(COALESCE(wr.absentees, '[]'::jsonb))) AS absent_count,
          COALESCE(wr.visitors_count, jsonb_array_length(COALESCE(wr.visitors, '[]'::jsonb))) AS visitor_count,
          wr.meeting_date
...
// Cell Overview Metrics for Dashboard
export const getCellOverviewCtrl = async (req, res) => {
  try {
    const church_id = req.user?.church_id;
    if (!church_id) return res.status(400).json({ error: 'Church not specified' });

    // Get cell statistics
    const cellStats = await db.query(`
      SELECT 
        COUNT(*) as total_cells,
        COUNT(CASE WHEN is_active = TRUE THEN 1 END) as active_cells,
        COUNT(CASE WHEN is_ready_for_multiplication = TRUE THEN 1 END) as ready_to_multiply,
        AVG(member_count) as avg_cell_size,
        MAX(member_count) as max_cell_size
      FROM (
        SELECT 
          cg.id,
          cg.is_active,
    ...
    const attendanceStats = await db.query(`
      ...
      FROM (
        SELECT 
          SUM(COALESCE(wr.total_cell_attendance, jsonb_array_length(COALESCE(wr.attendees, '[]'::jsonb)))) as attendance_count
        FROM weekly_reports wr
        WHERE wr.church_id = $1
          AND wr.meeting_date >= NOW() - INTERVAL '30 days'
          AND (wr.is_deleted IS NULL OR wr.is_deleted = FALSE)
        GROUP BY wr.cell_group_id
      ) attendance_data
    `, [church_id]);
    ...
    const upcomingMeetings = await db.query(`
      ...
      WHERE cg.church_id = $1
        ...
        AND cg.meeting_day IS NOT NULL
      ORDER BY cg.meeting_day
      LIMIT 5
    `, [church_id]);

    const stats = cellStats.rows[0] || {};
    const attendance = attendanceStats.rows[0] || {};

    return res.json({
      total_cells: parseInt(stats.total_cells) || 0,
      active_cells: parseInt(stats.active_cells) || 0,
      ready_to_multiply: parseInt(stats.ready_to_multiply) || 0,
      avg_cell_size: parseFloat(stats.avg_cell_size) || 0,
      max_cell_size: parseInt(stats.max_cell_size) || 0,
      avg_attendance: parseFloat(attendance.avg_attendance) || 0,
      max_attendance: parseInt(attendance.max_attendance) || 0,
      upcoming_meetings: upcomingMeetings.rows || []
    });
  } catch (err) {
    console.error('getCellOverviewCtrl error', err);
    return res.status(500).json({ error: 'Failed to fetch cell overview' });
  }
};
