...
export async function getDashboardMetrics(req, res) {
  try {
    const churchId = req.user?.church_id;
    if (!churchId) return res.status(400).json({ error: 'church_id required' });
    const data = await model.getDashboardMetrics(churchId);
    res.json(data);
  } catch (err) {
...
export async function getAdminSystemMetrics(req, res) {
  try {
    const data = await model.getAdminSystemMetrics();
    res.json(data);
  } catch (err) {
    console.error('getAdminSystemMetrics error', err);
    res.status(500).json({ error: err.message || 'Failed to fetch admin system metrics' });
  }
}