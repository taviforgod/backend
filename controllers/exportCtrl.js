...
export const exportExits = [
  ...
  async (req, res) => {
    try {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="inactive_exits_${format(new Date(), 'yyyyMMdd')}.csv"`);

      const headers = ['id','member_id','first_name','surname','exit_type','exit_reason','exit_date','is_suggestion','processed_by_email','notes','created_at'];
