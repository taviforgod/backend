...
export async function listExits(req, res) {
  try {
    ...
    if (includeInterviews) {
      const rows = await model.listExitsWithInterviews(church_id, { offset, limit, search, fromDate, toDate });
      const total_count = rows?.length ? (rows[0].total_count || rows.length) : 0;
      return res.json({ rows, total_count });
    } else {
      const rows = await model.listExits(church_id, { offset, limit, search, fromDate, toDate });
      // For the simple listExits we return rows and a total_count equal to length (or implement a separate count later)
      return res.json({ rows, total_count: Array.isArray(rows) ? rows.length : 0 });
    }
  } catch (err) {
    console.error(err);
    ...
    try {
      ...
      (async () => {
        try {
          ...
          const memberDisplayName = member ? `${member.first_name} ${member.surname}`.trim() : `Member ${member_id}`;

          const title = 'Inactive exit recorded';
          const message = `Exit recorded for ${memberDisplayName}${exitRow?.exit_reason ? ` (${exitRow.exit_reason})` : ''}.`;
          ...
          if (io) {
            ...
            // Explicit socket event for immediate client handling
            const payload = { exit_id: exitRow?.id ?? null, member_id, exit_type: exitRow?.exit_type ?? null };
            try {
              if (member_id) io.to(`member:${member_id}`).emit('exit:created', payload);
              if (church_id) io.to(`church:${church_id}`).emit('exit:created', payload);
              // broadcast on the server (fallback)
              io.emit && io.emit('exit:created', payload);
            } catch (emitErr) {
              console.warn('exit:created emit failed', emitErr?.message || emitErr);
            }
          }
        } catch (nErr) {
  ...
  try {
    ...
    (async () => {
      try {
        ...
        const message = `Exit ${id} was updated${memberDisplayName}.`;
  ...
  try {
    ...
    (async () => {
      try {
        ...
        const message = `Exit ${id} was removed${memberDisplayName}.`;
    ...
    try {
      ...
      (async () => {
        try {
          ...
          // Get member details for notification
          const member = await memberModel.getMemberById(memberId);
          const memberDisplayName = member ? `${member.first_name} ${member.surname}`.trim() : `Member ${memberId}`;

          const title = 'Member reinstated';
          const message = `${memberDisplayName} was reinstated from exit ${id}.`;
      ...
      (async () => {
        for (const exit of reinstated) {
          try {
            ...
            const title = 'Member reinstated';
            const message = `${memberDisplayName} was reinstated from exit ${exit.id}.`;
  ...
  try {
    ...
    (async () => {
      try {
        ...
        // Get member details for notification
        const member = await memberModel.getMemberById(fixed.member_id);
        const memberDisplayName = member ? `${member.first_name} ${member.surname}`.trim() : `Member ${fixed.member_id}`;

        const title = 'Exit record consistency fixed';
        const message = `Exit ${exit_id} for ${memberDisplayName} was marked as reinstated to fix data inconsistency.`;
