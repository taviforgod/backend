...
import * as visitorModel from '../models/visitorNameModel.js';
...
export const promoteToVisitor = async (req, res) => {
  const church_id = req.user?.church_id;
  const { contactId, cell_group_id } = req.body;
  ...
  member = await findMemberByContact({
    contact_primary: contact.contact_primary,
    email: contact.email,
    church_id
  });
  if (member) {
    flow = 'existing_member';
    await model.updateContact(contact.id, church_id, { status: 'linked_to_member' });
    return res.json({ member, contactConverted: true, flow });
  }

  // Check for existing visitor
  const existingVisitor = await visitorModel.findVisitorByContact(church_id, {
    contact_primary: contact.contact_primary,
    email: contact.email
  });

  if (existingVisitor) {
    visitor = await visitorModel.updateVisitorById(existingVisitor.id, church_id, {
    ...
    visitor = await visitorModel.createVisitor({
      church_id,
