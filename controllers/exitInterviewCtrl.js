    ...
    try {
      ...
      (async () => {
        try {
          ...
          const message = `Interview ${interviewRow.id} recorded and ${memberDisplayName} marked inactive.`;
          ...
          if (io) {
            ...
            // explicit exit:created event for clients
  ...
  try {
    ...
    (async () => {
      try {
        ...
        // Get member details for notification if member_id exists
        let memberDisplayName = '';
        if (member_id) {
          const member = await memberModel.getMemberById(member_id);
          memberDisplayName = member ? ` for member ${member.first_name} ${member.surname}`.trim() : ` for member ${member_id}`;
        }
        
        const title = 'Exit interview recorded';
        const message = `An exit interview was recorded${memberDisplayName}.`;
